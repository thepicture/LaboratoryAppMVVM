...
namespace LaboratoryAppMVVM.Models.Exports
{
    public class OrderContentDrawer : ContentDrawer
    {
        ...
        public override void Draw()
        {
            ...
            table.Cell(6, 2).Range.Text = _order.Patient.BirthDate
                .ToString("yyyy-MM-dd");
            table.Cell(7, 1).Range.Text = "Перечень услуг";
            table.Cell(7, 2).Range.Text = string
                .Join(", ", _order.AppliedService.ToList()
                .Select(s => s.Service.Name));
            table.Cell(8, 1).Range.Text = "Стоимость";
            table.Cell(8, 2).Range.Text = _order.AppliedService
        ...
        public override void Save()
        {
            try
            {
                string nameOfFile = "Заказ" + DateTime
